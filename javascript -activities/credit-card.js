...
function validateCred(arr) {
    ...
    let sumOne = newArr.reduce((acc, val) => {
        return acc + val;
    })
    let sumTwo = newArrTwo.reduce((acc, val) => {
        return acc + val;
    })
    let sumThree = sumOne + sumTwo;
    //console.log(sumOne)
    //console.log(sumTwo);
...
function idInvalidCompanies(nestedArr) {
    let newArr = [];
    for (var i = 0; i < nestedArr.length; i++) {
        if (nestedArr[i][0] === 3) {
            newArr.push("Amex");
...
let batched = findInvalidCards(batch);
console.log(idInvalidCompanies(batched));